...
CALL(0x00537D8F, _Write_PCX_File_hack_png);
...
Write_PCX_File_hack_png(CCFileClass *ccFile, DSurface *surface, void *palette)
{
    ...
    if (!initialized)
    {
        ...
        CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ScreenshotWriter, NULL, 0, NULL);
    ...
    ScreenshotBuffer = ac_malloc(ScreenshotHeight * (ScreenshotWidth * 2));
...
ScreenshotWriter()
{
    ...
    while (WaitForSingleObject(ScreenshotSignal, INFINITE) == WAIT_OBJECT_0)
    {
        ...
        char *rgb24buf = (char*)ac_malloc(ScreenshotHeight * (ScreenshotWidth * 3));
...
void *lodepng_malloc(size_t size) { return ac_malloc(size); }
void  lodepng_free(void *ptr) { ac_free(ptr); }
void *lodepng_realloc(void *ptr, size_t new_size) { return ac_realloc(ptr, new_size); }
